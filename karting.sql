...
INTO TABLE
Position (
PositionName,PositionDescription,PayPeriod,PayRate
)
select * from race

select ID_volunteer, Last_Name as'Фамилия', First_Name as 'Имя', country_name as'Страна', gender_name as'Пол' from volunteer join Gender on Gender_ID = ID_Gender join country on volunteer.ID_Country = country.ID_country

SELECT RaceTime as 'Время', concat(Racer.First_Name, ' ', Racer.Last_Name) as'Гонщик', country.ID_Country as'Страна' from Event_type join Event on ID_EventType = ID_Event join Result on result.ID_event = event.ID_event join Registration on result.ID_Registration = registration.ID_registration join Racer on registration.ID_racer = racer.ID_Racer join Gender on racer.Gender = ID_Gender join Race on event.ID_Race=race.ID_Race join Country on country.ID_Country=racer.ID_Country where racer.ID_country ='GER'

LOAD DATA INFILE
'volunteers.txt'
INTO TABLE
volunteer (
ID_Volunteer,First_Name,Last_Name,ID_Country,Gender_ID
)

insert into user values ('aboba@mail.ru','1234','aboba','abobovich','A');
select * from charity;
use kartskills;

create or replace table Sponsor_Racer
(
ID_Sponsor_Racer int not null,
ID_Racer int not null,
ID_Sponsorship int not null
);
ALTER TABLE `Sponsor_Racer`
ADD PRIMARY KEY (`ID_Sponsor_Racer`),
ADD KEY `ID_Racer` (`ID_Racer`),
ADD KEY `ID_Sponsorship` (`ID_Sponsorship`);

ALTER TABLE racer ADD COLUMN Photo varchar (250) null AFTER ID_Country;
...
INTO TABLE
Staff (
Staffid,FirstName,LastName,DateOfBirth,Gender,Positionid,Email
)
select * from racer
LOAD DATA INFILE
'TimeSheet.txt'
INTO TABLE
TimeSheet (
TimeSheetid,Staffid,StartDateTime,EndDateTime,PayAmmount
)
...
--
#select ID_Role from user  where Email = 'gord' and Password = '123'
INSERT INTO `user` (`Email`, `Password`, `First_Name`, `Last_Name`, `ID_Role`) VALUES
#('gorddd', '123', 'Dima', 'Dimov', 'C');
#('gordd', '123', 'Dima', 'Dimov', 'A');
#('gord', '123', 'Dima', 'Dimov', 'R');
update Racer set First_Name = '{FirstNameTB.Text}', Last_Name = '{LastNameTB.Text}', Gender = '{GenderID}', DateOfBirth = '{BirthDateDP.Text}', ID_Country = '{CountryID}', Photo = '{SelectedImage.Replace($@"","-")}' where  ID_Racer = {idracer}
update User set First_Name = '{FirstNameTB.Text}', Last_Name = '{LastNameTB.Text}', Password = '{Password1PB.Text}' where Email = '{EmailTB.Content.ToString()}'
update ()
